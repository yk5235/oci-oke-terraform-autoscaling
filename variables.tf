...
# Add these to your root variables.tf file
# ============================================

variable "enable_autoscaling" {
  description = "Enable cluster autoscaling for the autoscaling node pool"
  type        = bool
  default     = true
}
...
variable "eviction_grace_duration" {
  description = "Duration to wait before evicting pods from a node during scale down (format: PT#H#M#S)"
  type        = string
  default     = "PT1H"
  validation {
    condition     = can(regex("^PT([0-9]+H)?([0-9]+M)?([0-9]+S)?$", var.eviction_grace_duration))
    error_message = "eviction_grace_duration must be in ISO 8601 duration format (e.g., PT1H, PT30M, PT1H30M)."
  }
}
